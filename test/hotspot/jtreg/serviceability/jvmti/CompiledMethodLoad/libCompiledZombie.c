...
void JNICALL VMInit(jvmtiEnv* jvmti, JNIEnv* jni, jthread thread) {
  ...
  jmethodID thread_constructor = (*jni)->GetMethodID(jni, thread_class, "<init>", "()V");
  jthread agent_thread = (*jni)->NewObject(jni, thread_class, thread_constructor);

    (*jvmti)->RunAgentThread(jvmti, agent_thread, GenerateEventsThread, NULL, JVMTI_THREAD_NORM_PRIORITY);
}
