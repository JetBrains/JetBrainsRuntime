 ...
 * @run main/othervm -Djdk.nio.maxCachedBufferSize=0 -XX:MaxDirectMemorySize=10 LimitDirectMemory true 10 1
 * @run main/othervm -Djdk.nio.maxCachedBufferSize=0 -XX:MaxDirectMemorySize=1k LimitDirectMemory true 1k 100
 * @run main/othervm -Djdk.nio.maxCachedBufferSize=0 -XX:MaxDirectMemorySize=10m LimitDirectMemory true 10m 10m
 *
 * @summary Test: We can increase the amount of available memory
 * @run main/othervm -Djbr.java.io.use.nio=false -XX:MaxDirectMemorySize=65M LimitDirectMemory false 64M 65M
 *
 * @summary Test: Exactly the default amount of memory is available
 * @run main/othervm LimitDirectMemory false 10 1
 * @run main/othervm -Djbr.java.io.use.nio=false -Xmx64m LimitDirectMemory false 0 DEFAULT
 * @run main/othervm -Djbr.java.io.use.nio=false -Xmx64m LimitDirectMemory true 0 DEFAULT+1
 *
 * @summary Test: We should be able to eliminate direct memory allocation entirely
 * @run main/othervm -Djbr.java.io.use.nio=false -XX:MaxDirectMemorySize=0 LimitDirectMemory true 0 1
 *
 * @summary Test: Setting the system property should not work so we should be able
 *                to allocate the default amount
 * @run main/othervm -Djbr.java.io.use.nio=false -Dsun.nio.MaxDirectMemorySize=1K -Xmx64m
