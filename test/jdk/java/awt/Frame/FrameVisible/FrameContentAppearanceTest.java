...
public class FrameContentAppearanceTest extends Frame {
    ...
    public static void main(String[] args) throws Exception {
        ...
        if (imgFailCount > 0 || rectFailCount > 0 || ovalFailCount > 0 || aaOvalFailCount > 0) {
            String report = ((imgFailCount > 0) ? imgFailCount + " image rendering failure(s)\n" : "") +
            ...
            if (imgFailCount > FAIL_TOLERANCE || rectFailCount > FAIL_TOLERANCE ||
                ovalFailCount > FAIL_TOLERANCE || aaOvalFailCount > FAIL_TOLERANCE)
            {
                throw new RuntimeException("\nFailed:\n" + report);
            } else {
                System.out.println("Passed. Failures below the tolerance:\n" + report);
            }
        } else {
