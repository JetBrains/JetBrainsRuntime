...
public class WLSetBackground {
    ...
    public static void main(String[] args)  throws Exception {
        ...
        check(r, window, SystemColor.window);
        ...
        window.setVisible(true);
        check(r, window, Color.BLUE);

        System.out.println("Color: green, size 200x200");
        window.setSize(200, 200);
        window.setBackground(Color.GREEN);
        window.repaint();
        check(r, window, Color.GREEN);

        System.out.println("Color: red, size 200x200");
        window.setBackground(Color.RED);
        window.repaint();
        check(r, window, Color.RED);
    }

    public static void check(Robot r, Frame window, Color expectedBgColor) {
        r.waitForIdle();
        r.delay(PAUSE_MS);

        Point p = window.getLocationOnScreen();
        Color c = r.getPixelColor(p.x + 50, p.y + 50);
