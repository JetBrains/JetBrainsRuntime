...
public class Throwable implements Serializable {
    ...
    private void printStackTrace(PrintStreamOrWriter s) {
        ...
        final boolean showCoroutines = System.getProperty("kotlinx.coroutines.debug.print.stack.trace") != null;
        if (showCoroutines)
            dumpAllCoroutines(s);
    }
    ...
    private static void dumpAllCoroutines(PrintStreamOrWriter s) {
        final String debugDump = getCoroutinesDebugDump();
        if (debugDump != null)
            s.println(debugDump);
    }

    private static String getCoroutinesDebugDump() {
        if (!Holder.isInitialized) {
            return null;
        }

        try {
            final var instanceField = Holder.debugProbesImplClass.getField("INSTANCE");
            final var instance = instanceField.get(null);
            final var dumpMethod = Holder.debugProbesImplClass.getMethod("dumpCoroutines", java.io.PrintStream.class);
            final ByteArrayOutputStream byteStream = new ByteArrayOutputStream();
            final PrintStream ps = new PrintStream(byteStream, true, StandardCharsets.UTF_8);
            dumpMethod.invoke(instance, ps);
            return byteStream.toString(StandardCharsets.UTF_8);
        } catch (Exception e) {
            System.out.println("Error from CoroutineDumper" + e); // TODO: remove after debugging
        }
