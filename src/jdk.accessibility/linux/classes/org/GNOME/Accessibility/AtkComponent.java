...
public class AtkComponent {
    ...
    private AtkComponent(AccessibleContext ac) {
        ...
        this.accessibleComponentWeakRef = new WeakReference<AccessibleComponent>(ac.getAccessibleComponent());
    }
     ...
     *
     * @param ac
     * @param accessibleComponent
     * @param coordType
     * @return
     */
    public static Point getLocationByCoordinateType(AccessibleContext ac, AccessibleComponent accessibleComponent, int coordType) {
        assert EventQueue.isDispatchThread();
        ...
        if (coordType == AtkCoordType.WINDOW) {
            ...
            if (windowLocation == null)
                return null;
            Point componentLocation = accessibleComponent.getLocationOnScreen();
            if (componentLocation == null)
                return null;
            componentLocation.translate(-windowLocation.x, -windowLocation.y);
    ...
    private boolean contains(int x, int y, int coordType) {
        ...
        return AtkUtil.invokeInSwingAndWait(() -> {
            if (accessibleComponent.isVisible()) {
                Point componentLocation = getLocationByCoordinateType(ac, accessibleComponent, coordType);
                if (componentLocation == null) {
                    return false;
                }
