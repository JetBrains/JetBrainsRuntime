...
void Arguments::set_unrecognized_vm_options_property() {
  if (_unrecognized_vm_options != NULL) {
    ...
    SystemProperty* prop = new SystemProperty("java.vm.unrecognized.option", "", true, false);

    for (int i = 0; i < num_of_entries; i++) {
      const char* option_string = (_unrecognized_vm_options->at(i));
      prop->append_writeable_value(option_string);
    }
