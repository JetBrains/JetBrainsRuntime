...
static void print_process_memory_usage_platform(outputStream *st)
{
  ...
  if (os::Linux::query_process_memory_info(&info)) {
    ssize_t phys_total_kb = os::physical_memory() >> 10;
    ssize_t phys_avail_kb = os::available_memory() >> 10;
...
static void print_process_memory_usage_platform(outputStream *st)
{
  ...
  if (ret != 0) {
    ssize_t rss_kb = pmex.WorkingSetSize >> 10;
    ssize_t phys_total_kb = os::physical_memory() >> 10;
    ssize_t phys_avail_kb = os::available_memory() >> 10;
...
static void print_process_memory_usage_platform(outputStream *st)
{
  ...
  if (ret == KERN_SUCCESS) {
    ssize_t rss_kb = info.resident_size >> 10;
    ssize_t phys_total_kb = os::physical_memory() >> 10;
    ssize_t phys_avail_kb = os::available_memory() >> 10;
    int rss_percentile = (int)(rss_kb * 100.0 / phys_total_kb);
    st->print_cr("Resident Set Size: " SSIZE_FORMAT "K (%d%% of "
