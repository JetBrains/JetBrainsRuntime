...
void VM_EnhancedRedefineClasses::doit() {
  ...
  if (_object_klass_redefined) {
    for (int i = T_BOOLEAN; i < T_LONG+1; i++) {
      TypeArrayKlass* array_klass = TypeArrayKlass::cast(Universe::typeArrayKlass((BasicType)i));
    ...
    Universe::objectArrayKlass()->append_to_sibling_list();
  }
...
void VM_EnhancedRedefineClasses::flush_dependent_code() {
  ...
  bool instance_modified = (_max_redefinition_flags & Klass::ModifyInstances) != 0;
  if (JvmtiExport::all_dependencies_are_recorded() /*&& !instance_modified*/) {
