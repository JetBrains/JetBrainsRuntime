...
void VKDevice_CheckAndAdd(VKInstance* instance, VkPhysicalDevice physicalDevice) {
    ...
    instance->vkGetPhysicalDeviceFeatures2(physicalDevice, &deviceFeatures2);

    VkPhysicalDeviceProperties2 deviceProperties2 = {.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROPERTIES_2 };
    instance->vkGetPhysicalDeviceProperties2(physicalDevice, &deviceProperties2);
    ...
    for (uint32_t j = 0; j < queueFamilyCount; j++) {
        ...
        presentationSupported =
            instance->vkGetPhysicalDeviceWaylandPresentationSupportKHR(physicalDevice, j, instance->waylandDisplay);
#endif
        char logFlags[5] = {
    ...
    uint32_t layerCount;
    VK_IF_ERROR(instance->vkEnumerateDeviceLayerProperties(physicalDevice, &layerCount, NULL)) return;
    VkLayerProperties layers[layerCount];
    VK_IF_ERROR(instance->vkEnumerateDeviceLayerProperties(physicalDevice, &layerCount, layers)) return;
    ...
    VK_IF_ERROR(instance->vkEnumerateDeviceExtensionProperties(physicalDevice, NULL, &extensionCount, NULL)) return;
    VkExtensionProperties extensions[extensionCount];
    VK_IF_ERROR(instance->vkEnumerateDeviceExtensionProperties(physicalDevice, NULL, &extensionCount, extensions)) return;
    ...
    ARRAY_PUSH_BACK(instance->devices) = (VKDevice) {
        .name = deviceName,
...
const char* VKFunctionTable_InitDevice(VKInstance* instance, VKDevice* device);
...
/*
 * Class:     sun_java2d_vulkan_VKDevice
 * Method:    reset
 * Signature: (J)void
 */
JNIEXPORT void JNICALL
Java_sun_java2d_vulkan_VKDevice_reset(JNIEnv *env, jclass jClass, jlong jDevice) {
 ...
 */
JNIEXPORT void JNICALL
Java_sun_java2d_vulkan_VKDevice_init(JNIEnv *env, jclass jClass, jlong jDevice) {
    VKDevice* device = jlong_to_ptr(jDevice);
    if (device == NULL) {
        JNU_ThrowByName(env, "java/lang/IllegalStateException", "jDevice is NULL");
        return;
    ...
    VKInstance* instance = VKInstance_GetInstance();
    VK_IF_ERROR(instance->vkCreateDevice(device->physicalDevice, &createInfo, NULL, &device->handle)) {
    ...
    const char* missingAPI = VKFunctionTable_InitDevice(instance, device);
    if (missingAPI != NULL) {
        VKDevice_Reset(device);
