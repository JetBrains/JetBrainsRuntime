...
static VKRenderingContext context = {
        ...
        .transformModCount = 1,
        .constants = {
            .transform = J2D_VK_ID_TRANSFORM_INITIALIZER,
            .composite = { 0, 1.0f }
        },
        .clipModCount = 1,
        .clipRect = J2D_VK_NO_CLIP_RECT_INITIALIZER,
...
void VKRenderer_CreateImageDescriptorSet(VKRenderer* renderer, VkDescriptorPool* descriptorPool, VkDescriptorSet* set) {
    VKDevice* device = renderer->device;
    for (ptrdiff_t i = (ptrdiff_t)renderer->imageDescriptorPools.size - 1; i >= 0; i--) {
...
void VKRenderer_FlushSurface(VKSDOps* surface) {
    ...
    if (surface->drawableType == VKSD_WINDOW) {
        ...
        // Do blit.
        VkExtent2D intersection = VKUtil_IntersectVkExtent2D(surface->image->extent, win->swapchainExtent);
        VkOffset3D intersectionBottomRight = {.x = (int32_t)intersection.width, .y = (int32_t)intersection.height, .z = 1};
        VkImageBlit blit = {
                .srcSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 },
                .srcOffsets[0] = {0, 0, 0},
                .srcOffsets[1] = intersectionBottomRight,
                .dstSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 },
                .dstOffsets[0] = {0, 0, 0},
                .dstOffsets[1] = intersectionBottomRight,
        };
...
void VKRenderer_BlitBackingImageContentOnto(VKSDOps *surface, VKImage *targetImage) {
    ...
    VkExtent2D intersection = VKUtil_IntersectVkExtent2D(surface->image->extent, targetImage->extent);
    VkOffset3D intersectionBottomRight = {.x = (int32_t)intersection.width, .y = (int32_t)intersection.height, .z = 1};
    VkImageBlit blit = {
            .srcSubresource = { VK_IMAGE_ASPECT_COLOR_BIT, 0, 0, 1 },
            .srcOffsets[0] = {0, 0, 0},
...
void VKRenderer_ConfigureSurface(VKSDOps* surface, VkExtent2D extent, VKDevice* device) {
    ...
    // Unused here, because we're updating these synchronously
    (void) extent;
    (void) device;
