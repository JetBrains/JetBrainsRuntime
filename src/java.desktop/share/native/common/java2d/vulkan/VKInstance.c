...
static void VKInstance_Destroy(VKInstance* instance) {
    if (instance == NULL) return;

    if (instance->devices != NULL) {
        for (uint32_t i = 0; i < ARRAY_SIZE(instance->devices); i++) {
            VKDevice_Reset(&instance->devices[i]);
        }
        ARRAY_FREE(instance->devices);
    }

#if defined(DEBUG)
    if (instance->vkDestroyDebugUtilsMessengerEXT != NULL && instance->handle != VK_NULL_HANDLE) {
        instance->vkDestroyDebugUtilsMessengerEXT(instance->handle, instance->debugMessenger, NULL);
    }
#endif

    VKComposites_Destroy(instance->composites);

    if (instance->vkDestroyInstance != NULL) {
        instance->vkDestroyInstance(instance->handle, NULL);
    }
    free(instance);
    J2dRlsTraceLn(J2D_TRACE_INFO, "VKInstance_Destroy");
}
...
const char* VKFunctionTable_InitInstance(PFN_vkGetInstanceProcAddr vkGetInstanceProcAddr, VKInstance* instance);

static VKInstance* VKInstance_Create(PFN_vkGetInstanceProcAddr vkGetInstanceProcAddr) {
...
//            VK_VALIDATION_FEATURE_ENABLE_DEBUG_PRINTF_EXT,
    ...
    if (instance == NULL) {
        J2dRlsTraceLn(J2D_TRACE_ERROR, "Vulkan: Cannot allocate VKInstance")
        ARRAY_FREE(enabledLayers);
        ARRAY_FREE(enabledExtensions);
        return NULL;
    }
    *instance = (VKInstance) {};
    ...
    VK_IF_ERROR(table.vkCreateInstance(&instanceCreateInfo, NULL, &instance->handle)) {
        ARRAY_FREE(enabledLayers);
        ARRAY_FREE(enabledExtensions);
        VKInstance_Destroy(instance);
        return NULL;
    }
    ...
    missingAPI = VKFunctionTable_InitInstance(vkGetInstanceProcAddr, instance);
    if (missingAPI != NULL) {}
#if defined(VK_USE_PLATFORM_WAYLAND_KHR)
    else if (instance->vkGetPhysicalDeviceWaylandPresentationSupportKHR == NULL)
        missingAPI = "vkGetPhysicalDeviceWaylandPresentationSupportKHR";
    else if (instance->vkCreateWaylandSurfaceKHR == NULL)
    ...
    instance->composites = VKComposites_Create();
    ...
    if (foundDebugLayer && foundDebugExt && pNext) {
        ...
        VK_IF_ERROR(instance->vkCreateDebugUtilsMessengerEXT(instance->handle, &debugUtilsMessengerCreateInfo,
                                                             NULL, &instance->debugMessenger)) {}
    }
#endif
...
static VkBool32 VKInstance_FindDevices(VKInstance* instance) {
    uint32_t count;
    VK_IF_ERROR(instance->vkEnumeratePhysicalDevices(instance->handle, &count, NULL)) return JNI_FALSE;
    J2dRlsTraceLn1(J2D_TRACE_INFO, "Vulkan: Found %d physical devices:", count)
    VkPhysicalDevice physicalDevices[count];
    VK_IF_ERROR(instance->vkEnumeratePhysicalDevices(instance->handle, &count, physicalDevices)) return JNI_FALSE;
    ARRAY_ENSURE_CAPACITY(instance->devices, count);
    for (uint32_t i = 0; i < count; i++) {
        VKDevice_CheckAndAdd(instance, physicalDevices[i]);
    }
    if (ARRAY_SIZE(instance->devices) == 0) {
...
static jobjectArray createJavaDevices(JNIEnv *env, VKInstance* instance) {
    jclass deviceClass = (*env)->FindClass(env, "sun/java2d/vulkan/VKDevice");
    ...
    jobjectArray deviceArray = (*env)->NewObjectArray(env, ARRAY_SIZE(instance->devices), deviceClass, NULL);
    if (deviceArray == NULL) return NULL;
    for (uint32_t i = 0; i < ARRAY_SIZE(instance->devices); i++) {
        jstring name = JNU_NewStringPlatform(env, instance->devices[i].name);
        if (name == NULL) return NULL;
        jobject device = (*env)->NewObject(env, deviceClass, deviceConstructor,
                                           ptr_to_jlong(&instance->devices[i]), name, instance->devices[i].type);
...
Java_sun_java2d_vulkan_VKInstance_initNative(JNIEnv *env, jclass wlge, jlong nativePtr) {
#ifdef DEBUG
    ...
    VKInstance* vk = VKInstance_Create(vkGetInstanceProcAddr);
    if (vk == NULL) {
        vulkanLibClose();
        return NULL;
    }
    ...
    if (!VKInstance_FindDevices(vk)) {
        VKInstance_Destroy(vk);
        vulkanLibClose();
        return NULL;
    }

    jobjectArray deviceArray = createJavaDevices(env, vk);
    if (deviceArray == NULL) {
        VKInstance_Destroy(vk);
        vulkanLibClose();
        return NULL;
