...
jint ATexturePoolHandle_GetActualWidth(ATexturePoolHandle *handle) {
    ...
    if (TRACE_USE_API) J2dRlsTraceLn1(J2D_TRACE_INFO, "ATexturePoolHandle_GetActualWidth: handle = %p", handle);
...
jint ATexturePoolHandle_GetActualHeight(ATexturePoolHandle *handle) {
    CHECK_NULL_RETURN(handle, 0);
    CHECK_NULL_RETURN(handle->_poolItem, 0);
    if (TRACE_USE_API) J2dRlsTraceLn1(J2D_TRACE_INFO, "ATexturePoolHandle_GetActualHeight: handle = %p", handle);
