...
public final class Win32VKGraphicsConfig extends Win32GraphicsConfig implements VKGraphicsConfig {

    private final VKGraphicsConfig offscreenConfig;

    @SuppressWarnings("deprecation")
    public Win32VKGraphicsConfig(VKGraphicsConfig offscreenConfig, Win32GraphicsDevice device, int visualnum) {
    ...
    public String toString() {
        return "Win32VKGraphicsConfig[" + descriptorString() + ", " + getDevice().getIDstring() + "]";
    }

    @Override
    public SurfaceData createSurfaceData(WComponentPeer peer, int numBackBuffers) {
        return new Win32VKWindowSurfaceData(peer);
    }
}
