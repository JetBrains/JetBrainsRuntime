...
initJavaRefs(JNIEnv *env) {
    ...
    CHECK_NULL_RETURN(stopRepeatMID = (*env)->GetMethodID(env, keyRepeatManagerClass, "stopRepeat", "(JI)V"), false);
    return true;
}
...
handleKey(long timestamp, uint32_t keycode, bool isPressed, bool isRepeat) {
    ...
    if (isPressed) {
        ...
        (*env)->CallVoidMethod(env, keyboard.keyRepeatManager, stopRepeatMID, timestamp, keycode);
        JNU_CHECK_EXCEPTION(env);
    }
}
