...
initJavaRefs(JNIEnv *env)
{
    ...
    CHECK_NULL_RETURN(setRepeatInfoMID = (*env)->GetMethodID(env, keyRepeatManagerClass, "setRepeatInfo", "(II)V"), false);
...
handleKey(long timestamp, uint32_t keycode, bool isPressed, bool isRepeat)
{
    ...
    // layout to the QWERTY key map. Hence, the 'qwertyKeysym <= 0xff' check.

    if (keyboard.useNationalLayouts && !keyboard.asciiCapable && qwertyKeysym <= 0xff) {
