...
/* BGRA glyph that is rendered using XRenderComposite instead of XRenderCompositeText32.
 * Used for colored glyphs */
...
Java_sun_java2d_xr_XRBackendNative_addBGRAGlyphImagesNative
        (JNIEnv* env, jclass clazz, jint drawable, jlongArray javaGlyphInfoPointersArray, jint glyphCnt, jlong format32) {
    jlong* glyphInfoPointers;
    if ((glyphInfoPointers = (jlong *)(*env)->GetPrimitiveArrayCritical(env, javaGlyphInfoPointersArray, NULL)) == NULL) return;
    ...
    for (int i = 0; i < glyphCnt; i++) {
        ...
        Pixmap pixmap = XCreatePixmap(awt_display, (Drawable) drawable, glyphInfo->width, glyphInfo->height, 32);
        GC gc = XCreateGC(awt_display, (Drawable) pixmap, 0L, NULL);
        XImage* image = XCreateImage(awt_display, NULL, 32, ZPixmap, 0, (char*) glyphInfo->image,
                                     glyphInfo->width, glyphInfo->height, 32, glyphInfo->rowBytes);
        XPutImage(awt_display, pixmap, gc, image, 0, 0, 0, 0, glyphInfo->width, glyphInfo->height);
        image->data = NULL;
        XDestroyImage(image);
        XFreeGC(awt_display, gc);
        Picture picture = XRenderCreatePicture(awt_display, pixmap, format, 0, &pictureAttributes);

        BGRAGlyphInfo* bgraGlyphInfo = (BGRAGlyphInfo*) malloc(sizeof(BGRAGlyphInfo));
    ...
    (*env)->ReleasePrimitiveArrayCritical(env, javaGlyphInfoPointersArray, glyphInfoPointers, JNI_ABORT);
}

JNIEXPORT void JNICALL
Java_sun_java2d_xr_XRBackendNative_freeBGRAGlyphImages
        (JNIEnv* env, jclass clazz, jlongArray javaGlyphInfoPointersArray, jint glyphCnt) {
    jlong* glyphInfoPointers;
    if ((glyphInfoPointers = (jlong *)(*env)->GetPrimitiveArrayCritical(env, javaGlyphInfoPointersArray, NULL)) == NULL) return;

    for (int i = 0; i < glyphCnt; i++) {
        BGRAGlyphInfo* bgraGlyphInfo = (BGRAGlyphInfo*) jlong_to_ptr(glyphInfoPointers[i]);
        ...
        if(glyphset == -1) { // BGRA glyph, render as image
            ...
            for (int ch = 0; ch < nchars; ch++) {
                BGRAGlyphInfo* bgraGlyphInfo = (BGRAGlyphInfo*)
                        (((jlong) xids[charCnt + ch * 2] << 32) | (((jlong) xids[charCnt + ch * 2 + 1]) & 0xFFFFFFFF));
                GlyphInfo* glyph = bgraGlyphInfo->glyphInfo;
                XRenderComposite(awt_display, PictOpOver, bgraGlyphInfo->picture, (Picture) 0, (Picture) dst,
                                 0, 0, 0, 0,
                                 (int) (x + glyph->topLeftX), (int) (y + glyph->topLeftY),
