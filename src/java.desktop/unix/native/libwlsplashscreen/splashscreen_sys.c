...
pointer_handle_enter(void *data, struct wl_pointer *pointer, uint32_t serial, struct wl_surface *surface,
    ...
    struct wl_cursor_image *image = splash->state->default_cursor->images[0];
    wl_pointer_set_cursor(pointer, serial, splash->state->cursor_surface,
...
seat_handle_capabilities(void *data, struct wl_seat *seat, enum wl_seat_capability caps) {
    wayland_state *wl_state = ((Splash*)(data))->state;
...
SplashCreateWindow(Splash * splash) {
    ...
    wl_seat_add_listener(splash->state->wl_seat, &wl_seat_listener, splash);
