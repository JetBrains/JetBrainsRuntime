...
public class WLComponentPeer implements ComponentPeer {
    ...
    WLComponentPeer(Component target) {
        ...
        wlSize.setFromJavaSize(size.width, size.height);
    ...
    void updateSurfaceData() {
        SurfaceData.convertTo(WLSurfaceDataExt.class, surfaceData).revalidate(
                getBufferWidth(), getBufferHeight(), -1 /* unused */);
    }
    ...
    private void setSizeTo(int newWidth, int newHeight) {
        ...
        if (isSizeBeingConfigured() && wlSize.hasPixelSizeSet()) {
            ...
            // some implementations (Weston) reacts badly when the size of the surface
            // mismatches the configured size and we can't always precisely derive the surface
            // size from the Java (client) size because of rounding errors.
            wlSize.setJavaSize(newSize.width, newSize.height);
        } else {
            wlSize.setFromJavaSize(newSize.width, newSize.height);
        }
    }
    ...
    private void changeSizeToConfigured(int newSurfaceWidth, int newSurfaceHeight) {
        wlSize.setFromSurfaceSize(newSurfaceWidth, newSurfaceHeight);
        int newWidth = surfaceUnitsToJavaUnits(newSurfaceWidth);
        int newHeight = surfaceUnitsToJavaUnits(newSurfaceHeight);
        try {
    ...
    private class WLSize {
        ...
        void setFromJavaSize(int width, int height) {
            synchronized (dataLock) {
        ...
        void setFromSurfaceSize(int width, int height) {
            synchronized (dataLock) {
