...
package sun.awt.wl.im.zwp_text_input_v3;
...
final class WLInputMethodZwpTextInputV3 extends InputMethodAdapter {
    ...
    private static final PlatformLogger log = PlatformLogger.getLogger("sun.awt.wl.im.zwp_text_input_v3.WLInputMethodZwpTextInputV3");
    ...
    private static void awtFillWlCursorRectangleOf(Component component, OutgoingChanges out) {
        assert(component != null);
        assert(out != null);

        assert(EventQueue.isDispatchThread());

        Rectangle cursorRectangle = awtGetWlCursorRectangleOf(component);
        // Double-check because it's too important not to put null into out.
        //noinspection ConstantValue
        if (cursorRectangle == null) throw new NullPointerException("awtGetWlCursorRectangleOf(component)");

        out.setCursorRectangle(cursorRectangle);
    }
    ...
    private void awtInstallIMHighlightingInto(
        ...
        //     already converted text (e.g. pinyin in case of pinyin->hanzi conversion),
        //     and RAW for text before the conversion gets applied (e.g. hanzi in case of pinyin->hanzi conversion).
        //
    ...
    private void wlEnableContextNow() {
        ...
        final var changeSet = new OutgoingChanges()
    ...
    boolean wlUpdateCursorRectangle(final boolean forceUpdate) {
        ...
        if ( newCursorRectangle != null && (forceUpdate || newCursorRectangleDiffers) ) {
            // TODO:                                                support .setTextChangeCause(...)
    ...
    private void zwp_text_input_v3_onEnter(long enteredWlSurfacePtr) {
        ...
        try {
            if (log.isLoggable(PlatformLogger.Level.FINER)) {
                log.finer("zwp_text_input_v3_onEnter(enteredWlSurfacePtr={0}).", enteredWlSurfacePtr);
            }
    ...
    private void zwp_text_input_v3_onLeave(long leftWlSurfacePtr) {
        ...
        try {
            if (log.isLoggable(PlatformLogger.Level.FINER)) {
                log.finer("zwp_text_input_v3_onLeave(leftWlSurfacePtr={0}).", leftWlSurfacePtr);
            }
    ...
    private void zwp_text_input_v3_onPreeditString(byte[] preeditStrUtf8, int cursorBeginUtf8Byte, int cursorEndUtf8Byte) {
        ...
        try {
            if (log.isLoggable(PlatformLogger.Level.FINER)) {
                log.finer("zwp_text_input_v3_onPreeditString(cursorBeginUtf8Byte={0}, cursorEndUtf8Byte={1}, preeditStrUtf8={2}).",
                          cursorBeginUtf8Byte, cursorEndUtf8Byte, Arrays.toString(preeditStrUtf8));
            }

            wlGetIncomingChanges().updatePreeditString(preeditStrUtf8, cursorBeginUtf8Byte, cursorEndUtf8Byte);

            if (log.isLoggable(PlatformLogger.Level.FINER)) {
                log.finer("zwp_text_input_v3_onPreeditString: this.wlIncomingChanges={0}.", this.wlIncomingChanges);
            }
        } catch (Exception err) {
    ...
    private void zwp_text_input_v3_onCommitString(byte[] commitStrUtf8) {
        ...
        try {
            if (log.isLoggable(PlatformLogger.Level.FINER)) {
                log.finer("zwp_text_input_v3_onCommitString(commitStrUtf8={0}).", Arrays.toString(commitStrUtf8));
            }

            wlGetIncomingChanges().updateCommitString(commitStrUtf8);

            if (log.isLoggable(PlatformLogger.Level.FINER)) {
                log.finer("zwp_text_input_v3_onCommitString: this.wlIncomingChanges={0}.", this.wlIncomingChanges);
            }
        } catch (Exception err) {
