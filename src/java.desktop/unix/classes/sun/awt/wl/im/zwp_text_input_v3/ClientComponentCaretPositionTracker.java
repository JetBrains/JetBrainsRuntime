...
class ClientComponentCaretPositionTracker implements ComponentListener, CaretListener, TextListener
{
    ...
    private static final PlatformLogger log = PlatformLogger.getLogger("sun.awt.wl.im.zwp_text_input_v3.ClientComponentCaretPositionTracker");
    ...
    public void stopTrackingCurrentComponent() {
        ...
        if (isTextListenerInstalled) {
            ...
            try {
                ...
                if (log.isLoggable(PlatformLogger.Level.WARNING)) {
                    log.warning(String.format("Exception while removing the text listener from %s.", trackedComponentStrong), err);
                }
            }
        }
        ...
        if (isCaretListenerInstalled) {
            ...
            try {
                ...
                if (log.isLoggable(PlatformLogger.Level.WARNING)) {
                    log.warning(String.format("Exception while removing the caret listener from %s.", trackedComponentStrong), err);
                }
            }
        }
        ...
        try {
            ...
            if (log.isLoggable(PlatformLogger.Level.WARNING)) {
                log.warning(String.format("Exception while removing the component listener from %s.", trackedComponentStrong), err);
            }
        }
