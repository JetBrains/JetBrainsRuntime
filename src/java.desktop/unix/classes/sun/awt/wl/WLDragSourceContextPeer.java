...
public class WLDragSourceContextPeer extends SunDragSourceContextPeer {
    ...
    private class WLDragSource extends WLDataSource {
        ...
        protected synchronized void handleDnDDropPerformed() {
            boolean success = action != 0 && mime != null;
            int javaAction = success ? WLDataDevice.waylandActionsToJava(action) : 0;
            var x = WLToolkit.getInputState().getPointerX();
            var y = WLToolkit.getInputState().getPointerY();
