...
public final class WLInputMethod extends InputMethodAdapter {
    ...
    private interface ZwpTextInputV3 {
        ...
        final class InputContextState
        {
            ...
            public InputContextState resetToInitialValues()
            {
                textChangeCause = INITIAL_VALUE_TEXT_CHANGE_CAUSE;
                contentHint = INITIAL_VALUE_CONTENT_HINT;
                contentPurpose = INITIAL_VALUE_CONTENT_PURPOSE;
    ...
    private void logIgnoredCallOnBrokenInstance(String methodName, Object... methodArgs) {
        if (log.isLoggable(PlatformLogger.Level.WARNING)) {
            final StringBuilder logFmtStr = new StringBuilder(128);

            logFmtStr.append("{1}(");
            final int argsCount = methodArgs == null ? 0 : methodArgs.length;
            if (argsCount > 0) {
                logFmtStr.append("{2}");
                for (int i = 1; i < argsCount; ++i) {
                    logFmtStr.append(", {").append(i + 2).append("}");
                }
            }
            logFmtStr.append("): the call is ignored since this WLInputMethod@{0} is broken.");

            log.warning(
                // "{1}({2}, {3}, ...): the call is ignored since this WLInputMethod@{0} is broken."
                logFmtStr.toString(),
                System.identityHashCode(this), methodName, methodArgs
            );
        }
    }
    ...
    private void wlSendPendingChangesNow() {
        ...
        if (Boolean.FALSE.equals(changesToSend.getEnabledState())) {
            ...
            // TODO: "After [...] disable request all state information is invalidated and needs to be resent by the client."
        }
    ...
    private void zwp_text_input_v3_onEnter(long enteredWlSurfacePtr) {
        ...
        // There may be some changes put off until the context "enters" a surface.
        wlSendPendingChangesNowOrLater();
    }
