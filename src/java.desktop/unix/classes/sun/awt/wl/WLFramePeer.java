...
public class WLFramePeer extends WLDecoratedPeer implements FramePeer {
    ...
    private int widthBeforeMaximized; // Only used on EDT
    private int heightBeforeMaximized; // Only used on EDT
...
//    }
    ...
    public void setMaximizedBounds(Rectangle bounds) {
        // Ignored as some Wayland implementations (Weston) react badly when the size
        // of a maximized surface differs from the size suggested by Wayland through
        // the "configure" event.
    }
    ...
    void notifyConfigured(int newSurfaceX, int newSurfaceY, int newSurfaceWidth, int newSurfaceHeight,
        ...
        int newState = maximized ? Frame.MAXIMIZED_BOTH : Frame.NORMAL;
        int oldState = getState();
        synchronized (getStateLock()) {
            state = newState;
        }

        AWTAccessor.getFrameAccessor().setExtendedState(getFrame(), newState);
