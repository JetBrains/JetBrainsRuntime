    ...
    public void drawGlyphVector(final SunGraphics2D sg2d, final GlyphVector gV, final float x, final float y) {
        ...
        if (hasSlotData(gV, f2d)) {
            ...
            while (start < length) {
                ...
                Font f = new Font(slotInfo.font.getFontName(null),
                        gvFont.getStyle(), gvFont.getSize());
                if (gvFont.isTransformed()) {
                    f = f.deriveFont(gvFont.getTransform());
                }
                if (gvFont.hasLayoutAttributes()) {
                    f = f.deriveFont(gvFont.getAttributes());
                }
                sg2d.setFont(f);
