...
class CAccessibility implements PropertyChangeListener {
    ...
    public static Object[] getChildrenAndRolesRecursive(final Accessible a, final Component c, final int whichChildren, final boolean allowIgnored, final int level) {
        if (a == null) return null;
        ArrayList<Object> allCurrentLevelChildren = new ArrayList<Object>();
        allCurrentLevelChildren.addAll(Arrays.asList(getChildrenAndRoles(a, c, JAVA_AX_ALL_CHILDREN, allowIgnored)));
        ArrayList<Object> allChildren = new ArrayList<Object>();
        for (int i = 0; i < allCurrentLevelChildren.size(); i += 2) {
            if ((((Accessible) allCurrentLevelChildren.get(i)).getAccessibleContext().getAccessibleStateSet().contains(AccessibleState.SELECTED) && (whichChildren == JAVA_AX_SELECTED_CHILDREN)) ||
                    (((Accessible) allCurrentLevelChildren.get(i)).getAccessibleContext().getAccessibleStateSet().contains(AccessibleState.VISIBLE) && (whichChildren == JAVA_AX_VISIBLE_CHILDREN)) ||
                    (whichChildren == JAVA_AX_ALL_CHILDREN)) {
                allChildren.add(allCurrentLevelChildren.get(i));
                allChildren.add(allCurrentLevelChildren.get(i + 1));
                allChildren.add(String.valueOf(level));
            }
            if (getAccessibleStateSet(((Accessible) allCurrentLevelChildren.get(i)).getAccessibleContext(), c).contains(AccessibleState.EXPANDED)) {
                allChildren.addAll(Arrays.asList(getChildrenAndRolesRecursive(((Accessible) allCurrentLevelChildren.get(i)), c, whichChildren, allowIgnored, level + 1)));
            }
        }
