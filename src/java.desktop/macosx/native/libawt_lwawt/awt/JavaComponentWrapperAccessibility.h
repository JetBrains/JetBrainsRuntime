 ...
 * These elements are required by the native platform in order for a11y to work properly.
 * The JavaComponentWrapperAccessibility interface provides a concept of wrapping
 * an element originated from java (like a list item, or a table element) with a component
 * which has a11y role required by the native platform (like NSAccessibilityRowRole,
 * or NSAccessibilityCellRole) but does not have peer in java.
 *
 ...
 * created (see [JavaComponentAccessibility createWithParent]). When the wrapping component is
 * deallocated (as triggered from the java side) it releases the wrapped child.
 */
@interface JavaComponentWrapperAccessibility : JavaComponentAccessibility

@property (nonatomic, retain) JavaComponentAccessibility *wrappedChild;
