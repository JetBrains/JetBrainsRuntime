 ...
 * for clicks.
 */
static inline void autoDelay(JNIEnv *env, BOOL isMove) {
    if (!isMove){
    ...
    NSTimeInterval ti = 0.050;
    NSString *safeDelay = [PropertiesUtilities javaSystemPropertyForKey:@"sun.awt.osx.RobotSafeDelayMillis" withEnv:env];
    if (safeDelay != nil) {
        ti = (NSTimeInterval)[safeDelay intValue]/1000;
    }
    gNextKeyEventTime = [[NSDate date] timeIntervalSinceReferenceDate] + ti;
}
