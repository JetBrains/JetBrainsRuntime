...
void MTLRenderQueue_CheckPreviousOp(jint op) {
    ...
    if (0) J2dRlsTraceLn2(J2D_TRACE_INFO, "MTLRenderQueue_CheckPreviousOp: op=%s\tnew op=%s",
    ...
    @autoreleasepool {
        ...
        @try {
            while (b < end) {
                ...
                if (0) J2dRlsTraceLn1(J2D_TRACE_VERBOSE, "MTLRenderQueue_flushBuffer: opcode=%s", mtlOpCodeToStr(opcode));
                ...
                switch (opcode) {
                    ...
                    case sun_java2d_pipe_BufferedOpCodes_SET_SURFACES:
                    {
                        ...
                        const MTLContext *prevMtlc = mtlc;
                        ...
                        if (prevMtlc != NULL) {
                            if (FORCE_FLUSH_GLYPH_CACHE) {
                                ...
                                [prevMtlc.glyphCacheAA free];
                                [prevMtlc.glyphCacheLCD free];
                            } else {
                                // flush glyph / vertex caches if metal contexts (ie mtl device) are different:
                                [prevMtlc.glyphCacheAA flush];
                                [prevMtlc.glyphCacheLCD flush];

                                if (prevMtlc != mtlc) {
                                    if (0) J2dRlsTraceLn2(J2D_TRACE_VERBOSE, "SET_SURFACES: flush different device ? %p <=> %p", prevMtlc.device, mtlc.device);
                                    MTLVertexCache_ReleaseMaskCache(prevMtlc); // shared state !
                                }
                            }
                            if (isDisplaySyncEnabled() && IS_OUTPUT_DEST(dstOps)) {
                                [prevMtlc commitCommandBuffer:YES display:YES];
                                sync = NO;
                            } else {
                                // flush pending commands anyway:
                                [prevMtlc commitCommandBuffer:NO display:NO];
                            }
                        }
                    ...
                    case sun_java2d_pipe_BufferedOpCodes_SET_SCRATCH_SURFACE:
                    {
                        ...
                        if (prevMtlc != NULL) {
                            if (FORCE_FLUSH_GLYPH_CACHE) {
                                ...
                                if (prevMtlc != mtlc) {
                                    if (0) J2dRlsTraceLn2(J2D_TRACE_VERBOSE, "SET_SCRATCH_SURFACE: flush different device ? %p <=> %p", prevMtlc.device, mtlc.device);
                                    MTLVertexCache_ReleaseMaskCache(prevMtlc); // shared state !
                                }
                            }
                            // flush pending commands anyway:
                    ...
                    case sun_java2d_pipe_BufferedOpCodes_DISPOSE_CONFIG:
                    {
                        ...
                        MTLVertexCache_ReleaseMaskCache(mtlc); // shared state !
        ...
        } // Mimics JNI_COCOA_EXIT():
        @catch (NSException* e) {
            // TODO: report that in JVM crash log
             NSLog(@"MTLRenderQueue_flushBuffer: failed op=%s ctx=%p", mtlOpCodeToStr(opcode), mtlc);
             NSLog(@"MTLRenderQueue_flushBuffer Exception: %@", [e description]);
             NSLog(@"MTLRenderQueue_flushBuffer callstack: %@", [e callStackSymbols]);
             caughtException = e;
        }

        // Ensure flushing encoder before releasing pool:
        if (mtlc != NULL) {
