...
- (void) blitTexture {
    ...
    @autoreleasepool {
        ...
        if (directBlit) {
            J2dTraceLn1(J2D_TRACE_VERBOSE, "MTLLayer_blitTexture: direct blit frame=%d", fCount);
            ...
            J2dTraceLn1(J2D_TRACE_VERBOSE, "MTLLayer_blitTexture: store frame to buffer %d", fCount);
            ...
            if (blitFromBuf) {
                ...
                J2dTraceLn1(J2D_TRACE_VERBOSE, "MTLLayer_blitTexture: blit frame %d from buffer",
                            framesNumbers[outFramesIndex]);
        ...
        [commandBuf addCompletedHandler:^(id <MTLCommandBuffer> commandBuf) {
            if (frame == framesNumbers[outFramesIndex]) {
...
- (void) startDisplayLink {
    ...
    displayLinkCount += 2; // Keep alive displaylink counter
}
...
- (void) validateEnv:(JNIEnv*)env sData:(jobject)surfaceData {
    J2dTraceLn(J2D_TRACE_VERBOSE, "MTLLayer_validateEnv() called");
    if (surfaceData != NULL) {
