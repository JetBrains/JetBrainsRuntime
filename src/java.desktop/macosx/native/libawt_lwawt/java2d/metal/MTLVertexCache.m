...
MTLVertexCache_FlushGlyphVertexCache(MTLContext *mtlc)
{
    ...
    if (vertexCacheIndex > 0 && mtlc.glyphCacheAA != NULL) {
        id<MTLRenderCommandEncoder> gcEncoder = mtlc.glyphCacheAA->encoder;
        [gcEncoder setVertexBytes: vertexCache length:vertexCacheIndex * sizeof(J2DVertex)
                          atIndex:MeshVertexBuffer];
        id<MTLTexture> glyphCacheTex = mtlc.glyphCacheAA->texture;
