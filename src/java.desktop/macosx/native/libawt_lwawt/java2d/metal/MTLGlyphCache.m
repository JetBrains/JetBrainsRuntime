                       ...
                       func:(MTLFlushFunc *)func
{
    ...
    if (TRACE_TEXTURE)J2dRlsTraceLn(J2D_TRACE_VERBOSE, "MTLGlyphCache.glyphCacheInitWidth: new cacheInfo=%p",
                                     _cacheInfo);
    ...
    if (TRACE_TEXTURE) J2dRlsTraceLn(J2D_TRACE_VERBOSE, "MTLGlyphCache.glyphCacheInitWidth: texHandle=%p",
    ...
    // storage = MTLStorageModeManaged
#endif
    if (TRACE_TEXTURE) J2dRlsTraceLn(J2D_TRACE_VERBOSE, "MTLGlyphCache.glyphCacheInitWidth: usage=%d storage=%d",
                                      _cacheInfo->texture.usage, _cacheInfo->texture.storageMode);

    if (TRACE_TEXTURE) J2dRlsTraceLn(J2D_TRACE_VERBOSE, "[%.6lf] MTLGlyphCache.glyphCacheInitWidth: texture: tex=%p, w=%d h=%d, pf=%d",
...
- (void) free
{
    ...
    if (TRACE_TEXTURE) J2dRlsTraceLn(J2D_TRACE_VERBOSE, "MTLGlyphCache.free: cacheInfo=%p",
                                      _cacheInfo);

#if (USE_TEXTURE_POOL == 1)
    if (TRACE_TEXTURE) J2dRlsTraceLn(J2D_TRACE_VERBOSE, "MTLGlyphCache.free: release texHandle=%p",
                                      _cacheInfo->texHandle);

    [_cacheInfo->texHandle releaseTexture];
    [_cacheInfo->texHandle release]; // free reference
#else
    if (TRACE_TEXTURE) J2dRlsTraceLn(J2D_TRACE_VERBOSE, "[%.6lf] MTLGlyphCache.free: free texture: tex=%p, w=%d h=%d, pf=%d",
