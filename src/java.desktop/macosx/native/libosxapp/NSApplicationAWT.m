...
#define MAX_RETRY_WRITE_EXCEPTION   2
#define MAX_WRITE_EXCEPTION         20
...
// log exception methods (internals)
                 ...
                 file:(const char*)file line:(int)line function:(const char*)function
{
    ...
    if (loggedException < MAX_LOGGED_EXCEPTION) {
        [NSApplicationAWT toString:exception prefix:prefix to:info
                              file:file line:line function:function];
...
+ (void) toString:(NSException *)exception prefix:(NSString *)prefix to:(NSMutableString *)info
             file:(const char*)file line:(int)line function:(const char*)function
{
    NSString *header = (file != nil) ?
            [NSString stringWithFormat: @"%@Exception (%s:%d %s):\n%@\n", prefix, file, line, function, exception]
          : [NSString stringWithFormat: @"%@Exception (unknown):\n%@\n", prefix, exception];
    [info appendString:header];

    const NSArray<NSString*> *stack = [exception callStackSymbols];
