...
if [ -d "$APPLICATION_PATH/Frameworks" ]; then
  find "$APPLICATION_PATH/Frameworks" \
...
if [ "$JB_SIGN" = true ]; then for f in \
  "Frameworks/cef_server.app/Contents/Frameworks" "Frameworks"; do
  if [ -d "$APPLICATION_PATH/$f" ]; then
...
if [ -d "$APPLICATION_PATH/Frameworks" ]; then
  find "$APPLICATION_PATH/Frameworks" -name '*.framework'  -maxdepth 1 | while read -r line
    do
      log "Checking '$line':"
